...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (moodBtns && moodBtns.length > 0 && moodLabel && readingList && moodContent) {
    ...
    if (closeMoodContent && moodContent) {
    ...
    if (saveBtn && textarea && status) {
        saveBtn.addEventListener('click', () => {
            ...
            if (user && user.username) {
                ...
                fetch(`http://localhost:3000/users/${user.username}/notes`, {
                    method: "POST",
                ...
                status.textContent = "Please log in to save your note.";
            }
        });
    }
    ...
    if (logoutBtn) {
      logoutBtn.addEventListener('click', () => {
        ...
        window.location.href = 'login.html'; // or your login/signup page
      });
    }
});
